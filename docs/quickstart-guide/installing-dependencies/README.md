...
Installers for these dependencies can be retrieved directly from the vendors, or, easily downloaded in this [Setup Files ZIP](https://drive.google.com/open?id=14_h--ZDo1l7bNSh6F9sXIiP1nQN2QTpR).
...
1. Acquire a copy of the JDK. A copy is provided in the `Setup Files ZIP` (`jdk-8u102-windows-x64.exe`).
...
1. Acquire a copy of Elasticsearch. A copy is provided in the `Setup Files ZIP` (`elasticsearch-5.5.0`). Alternatively, for as long as Elastic continues hosting v5.5, they have it available [here](https://www.elastic.co/downloads/past-releases/elasticsearch-5-5-0).
...
1. Acquire a copy of MongoDB v3.2. A copy is provided in the `Setup Files ZIP` (`Mongodb-win32-x86_64-2008plus-ssl-v3.2-latest-signed`).
